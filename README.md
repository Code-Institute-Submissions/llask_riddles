...
This flask web application presents a quiz game where you guess the answer to riddles. The rules
...
background textarea.) It is akin to a traffic light system red-amber-green. Cadet blue class is used as placeholder 
text for more definition. The text-primary class is chosen to state the score for a particular riddle. The guess 
the riddle button is the text-info class with white text. The nav bar and header text are strong, bold and white. 
Strong enough to be noticed but light enpugh to be subtle. It is the masthead background image that dominates the 
header section.  Buttons are simple and obvious.
...
for any dependencies that need to be installed to run it (for example flask needs to be installed), by checking 
the requirements.txt file. Then just select the run.py file and click run to view in the browser.        
...
[Creative Commons License](https://creativecommons.org/publicdomain/zero/1.0/deed.en)  for the image
